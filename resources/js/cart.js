        ...
        if (data.nouvelleQuantite > 1) {
            ...
            .then((articleResponse) => {
                if (!articleResponse.ok) {
                    throw new Error("La requête a échoué : " + articleResponse.status);
                }
                return articleResponse.json(); // Utilisez le mot-clé 'return' ici
            })
            .then((articleData) => { // Renommez la variable 'data' pour éviter le conflit
                ...
                new swal({ title: 'L\'article a été ajouté au panier.', });
